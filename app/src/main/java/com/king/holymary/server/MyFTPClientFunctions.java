package com.king.holymary.server;
...
public class MyFTPClientFunctions {
	...
	public boolean ftpUploadFile(File srcFilePath, String desFileName,
								 String desDirectory, Context context) {
		boolean status = false;
		try {
//			FileInputStream srcFileStream = new FileInputStream(srcFilePath);
			...
			BufferedInputStream srcFileStream = new BufferedInputStream(new FileInputStream(srcFilePath));
			// change working directory to the destination directory
			// if (ftpChangeDirectory(desDirectory)) {
			status = mFTPClient.storeFile(desFileName, srcFileStream);
			// }
